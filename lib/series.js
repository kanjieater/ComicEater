...
function getLocalMetaData(cleanedArchivePath, rootPath, context) {
  ...
  const seriesMapping = mapSeries(fileName, fileMetaData, folderMetaData, context);
  if (seriesMapping) {
...
function getFilePathMetaData(context) {
  const {
    archivePath, maintenanceFolder,
  } = context;
  const cleanedArchivePath = removeJunk(archivePath, context.junkToFilter);

  const contentMetaData = getLocalMetaData(cleanedArchivePath, maintenanceFolder, context);
...
async function moveArchiveToSeriesFolder(archivePath, outputFilePath, maintenanceFolder) {
  if (archivePath !== outputFilePath) {
    ...
    try {
      ...
      if (error.code === 'DUPE') {
        const dupePath = path.join(maintenanceFolder, 'potentialDupes', path.basename(archivePath));
        log.warn(`Moving to potential duplicates to: "${dupePath}, Archive already exists in the correct Series Folder: "${outputFilePath}", `);
        await moveFile(archivePath, dupePath);
      } else {
        throw error;
      }
    }
  } else {
    // TODO: investigate if this can actually be triggered here
...
async function moveIdentifiedSeriesToSeriesFolder(identifiedContext) {
  async function getResult(context) {
    const { archivePath, fileFormattedMetaData: { outputFilePath }, maintenanceFolder } = context;
...
async function identifySeriesFromRemoteSource(context) {
  ...
  let anilistData = {};
  if (context.offline) {
    log.info(`Skipping remote lookups for offline sources: "${context.archivePath}"`);
  } else {
    anilistData = await getAnilistData(updatedContext);
  }
...
async function getSeriesWithRemoteMetaData(contexts) {
  ...
  logSuggestedSeries(results, getUnityMap(results.successful));
  return results;
}
