...
function unifySeriesResults(contexts) {
  const unityMap = {};
  contexts.forEach((context) => {
    const key = context.volumeStartPath || context.rootPath;
    if (!key) {
      return;
    }
    if (unityMap[key]) {
      ...
      unityMap[key] = {
        unifiedSeriesName: context.contentMetaData.seriesName,
...
async function identifySeriesFromRemoteSource(context) {
  ...
  const anilistData = await getAnilistData(context);
  const contentMetaData = {
    ...updatedContext.contentMetaData,
    ...anilistData.formattedData,
  };
  return {
    ...updatedContext,
    contentMetaData,
...
async function getSeries(archivePaths) {
  ...
  results.successful = unifySeriesResults(results.successful);
