...
const { getPathData, preferNative } = require('./utils');
...
function getFilePathMetaData(context) {
  ...
  const formattedContentMetaData = getFormattedContentMetaData(
    contentMetaData,
    updatedContext,
  );
...
async function moveIdentifiedSeriesToSeriesFolder(identifiedContext) {
  async function getResult(context) {
    const { archivePath, formattedContentMetaData: { outputFilePath }, queuePath } = context;
...
function logSuggestedSeries(contexts) {
  contexts.forEach((context) => {
    const {
      archivePath, formattedContentMetaData, seriesRoot, queuePath,
    } = context;

    const { outputFilePath } = formattedContentMetaData;
