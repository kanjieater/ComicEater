...
const config = {
  ...
  filePatterns: ['{{seriesName}}第{{volumeNumber}}', '{{seriesName}}'],
  folderPatterns: [
...
test('getFileMetaData should parse volumes meta data', () => {
  expect(
    getFileMetaData('Yokoshimakensanwa v01w-02s', [
      '{{seriesName}} {{volumeNumber}}',
    ]),
  ).toStrictEqual({
...
test('getFileMetaData should parse config filePatterns into content meta data', () => {
  expect(
    getFileMetaData('Bentler Bentler 01w', ['{{seriesName}} {{volumeNumber}}']),
  ).toStrictEqual({
...
test('getFolderMetaData should parse config filePatterns into content meta data but not across paths', () => {
  expect(
    ...
    volumeNumber: 1,
    volumeRange: [{ volumeNumber: 1 }, { volumeNumber: 2 }],
  });
});

test('getFileMetaData should parse config filePatterns into content meta data for content with spaces', () => {
  expect(getFileMetaData('極東事変 第1巻', config.filePatterns)).toStrictEqual({
    seriesName: '極東事変',
    volumeNumber: 1,
  });
});
...
test('mapGlobToRegexStr should transform any end folder glob pattern into regex', () => {
  ...
  expect(getRegex(allMatches, patternTemplate)).toStrictEqual(
    /^\[(?<authors>(?:[^/]?)+)\](?<seriesName>(?:[^/]?)+)全(?<totalVolumes>(?:[^/]?)+)\/(?:.*)$/,
  );
});
...
test('getFolderMetaData should parse config folderPatterns into content meta data with parenthesis', () => {
  expect(
    getFolderMetaData('(星野之宣) 未来の二つの顔 全01巻/', [
      '({{authors}}){{seriesName}}全{{totalVolumes}}/**/*',
    ]),
  ).toStrictEqual({
    seriesName: '未来の二つの顔',
...
test('getFolderMetaData should parse config folderPatterns into content meta data in different paths but ignore end globs that are unmatched', () => {
  expect(
    getFolderMetaData('[星野之宣] 未来の二つの顔 全01巻/2024/asdf/asdfasdf/', [
      '[{{authors}}]{{seriesName}}全{{totalVolumes}}/{{publishYear}}/**/*',
    ]),
  ).toStrictEqual({
    seriesName: '未来の二つの顔',
...
test('getFolderMetaData should use lookaheads to get all characters up to the next non-pattern character', () => {
  expect(
    getFolderMetaData('[星野之宣] 未来 二つの顔 全01巻/2024/asdf/asdfasdf/', [
...
test('getFolderMetaData should allow for custom regex', () => {
  const datRegex = 'regex:^\\[(?<junk>[^\\]\\[]+?)\\]\\[(?<authors>[^\\]]+?)\\](?<seriesName>.+?)(?<volumeNumber>\\d[^\\/]+?)\\[(?<notes>[^\\]\\[]+?)\\]\\/(?:.*)';
  // [20221130][浦沢直樹] あさドラ!(7) [ビッグコミックススペシャル]
  expect(
    getFolderMetaData(
      '[20190930][浦沢直樹] あさドラ！2 [ビッグコミックススペシャル]/',
  ...
  expect(
    getFolderMetaData(
      '[20230915][岩宗治生] ウスズミの果て 2 [HARTA COMIX]/',
      // eslint-disable-next-line no-useless-escape
      [datRegex],
    ),
  ).toStrictEqual({
...
test('getFileMetaData should allow for custom regex with repeated chars (年)', () => {
  const datRegex = 'regex:^(?<seriesName>.+?)(?<publishYear>\\d\\d\\d\\d+?)年(?<volumeNumber>\\d+?)月号(?:.*)';
  // 月刊少年チャンピオン2024年01月号
  expect(
    getFileMetaData(
      '月刊少年チャンピオン2024年01月号',
...
test('getFolderMetaData should parse a single template with */', () => {
  expect(
    getFolderMetaData('[星野之宣] 未来 二つの顔 全01巻/2024/', [
      '[{{authors}}*/',
    ]),
  ).toStrictEqual({
    writer: ['星野之宣] 未来 二つの顔 全01巻'],
...
test('getFolderMetaData should parse a single template with /*', () => {
  expect(
    getFolderMetaData('[星野之宣] 未来 二つの顔 全01巻/asdf/2024/', [
      '**/*{{publishYear}}/',
    ]),
  ).toStrictEqual({
    publishYear: 2024,
...
test('getFolderMetaData should parse multiple authors by their delimeters', () => {
  expect(
    getFolderMetaData(
      '[星野之宣×kanjieater] 未来の二つの顔 全01巻/',
      config.folderPatterns,
      { splitAuthorsBy: config.splitAuthorsBy },
    ),
  ).toStrictEqual({
    seriesName: '未来の二つの顔',
    totalVolumes: 1,
    authors: ['星野之宣', 'kanjieater'],
    writer: ['星野之宣', 'kanjieater'],
  });
});
...
test('getFolderMetaData should parse config folderPatterns into content meta data with parenthesis', () => {
  expect(
    getFolderMetaData('邪剣さんはすぐブレる(1)/', config.folderPatterns),
  ).toStrictEqual({
...
test('find removes configured junk', () => {
  expect(
    removeJunk('./(一般コミック) Kararesu v01.cbz', config.junkToFilter),
