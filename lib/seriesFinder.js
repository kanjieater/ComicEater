...
const sanitizers = {
  issueNumber: sanitizeVolumes,
  volumeNumber: sanitizeVolumes,
  totalVolumes: sanitizeVolumes,
...
function getCaptureGroupSelector(templateMatch, templateKey, regStr) {
  ...
  let captureStyle = '.+?';
  if (bridgeChars) {
    const [, alreadyDoneOffset, capturedBridgeChars] = bridgeChars;
    const afterMatchSubstr = regStr.slice(alreadyDoneOffset?.length + templateMatch.length);
    const existsAfter = afterMatchSubstr.includes(capturedBridgeChars);
    if (capturedBridgeChars && existsAfter && capturedBridgeChars.includes('/')) {
      captureStyle = '.+?';
    } else if (capturedBridgeChars && !existsAfter) {
      captureStyle = `.+(?=${capturedBridgeChars}])`;
    } else {
      // Go up to last/next slash if it exists
      captureStyle = '.+(?=\\/|)';
    }
  }
...
function getFileMetaData(archivePath, patternTemplates) {
  let contentMetaData = {};
  patternTemplates.find((patternTemplate) => {
    ...
    contentMetaData = getGroupMatch(archivePath, regex, uniqueKeys);
