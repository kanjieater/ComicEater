...
function getCaptureGroupSelector(match, key, patternTemplate) {
...
function generateCaptureGroupRegex(allMatches, patternTemplate) {
  ...
  allMatches.forEach(([match, key]) => {
    const captureGroupSelector = getCaptureGroupSelector(match, key, patternTemplate);
    regex = regex.replace(match, captureGroupSelector);
  });
  return regex;
}
