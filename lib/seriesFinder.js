...
function getFileMetaData(archivePath, patternTemplates, context) {
  ...
  log.info(`Best Pattern Match: "${finalPattern}" for "${archivePath}": Matched Data: "${inspect(contentMetaData)}"`);
...
function mapSeries(fileMetaData, folderMetaData, context) {
  const mapping = context.seriesMapping;
  if (!mapping) {
    return null;
  }
  const existingMetadata = { ...folderMetaData, ...fileMetaData };
