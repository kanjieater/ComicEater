...
async function rateLimit(fn, fileName) {
  return SYNC(async () => {
    ...
    log.info(`Calling AniList for ${fileName} - rate limit is ${sleepTime}ms per call`);
...
async function callAnilist(search) {
  let response;
  try {
    ...
    // TODO: Do i need to reraise or retry 500s?
  }
...
function formatSeriesName(title, context) {
  ...
  let cleanTitle = preferNative(title?.native, latinVersion, context.contentMetaData.languageISO);
...
async function searchAnilist(searchInput) {
  ...
  const normalizeInput = searchInput.normalize('NFC');
  const mangaData = (await callAnilist(normalizeInput))?.data?.Media || {};
  return mangaData;
}
