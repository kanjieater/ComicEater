...
const { getImages, verifyImageCount, validateImageDir } = require('./image');
const {
  removeEmptyDirs, getChildDirs, rm, getTmpPath, moveFile, checkPathExists,
} = require('./files');
...
const {
  getNewFilePath, getArchives, validateImagesExist, zipArchive, extract7z,
} = require('./command');
...
async function verifyCBZContents(archivePath) {
  const extractPath = path.join('cbzTest', archivePath);
  const { fileDir } = getTmpPath(extractPath);
  log.info(`Testing the CBZ by extracting & validating "${fileDir}"`);
  try {
    await extract7z(archivePath, fileDir);
    await validateContents(fileDir, fileDir);
  } catch (e) {
    await rm(fileDir, false);
    throw new Error(e);
  }
  return rm(fileDir, false);
}
...
async function zipAndMove(
  ...
  await verifyCBZContents(zippedPath);
