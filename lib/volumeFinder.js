...
const { zipAndMove, getNewFilePath, getArchives } = require('./command');
...
async function getNestedFolder(context) {
  ...
  updatedContext.contentMetaData = {
    ...updatedContext.contentMetaData,
    ...getContentMetaData(volumeStartPath, context),
  };
...
async function cleanUpNestedVolume(context, extractedArchiveDir) {
  if (context.subSagaResults.unsuccessful.length !== 0) {
    throw new SubSagaError(
