...
async function getNestedFolder(context) {
  ...
  const globPath = glob.escapePath(resolve(volumeStartPath));
  const childDirs = await glob(`${globPath}/**/*`, { onlyDirectories: true, deep: 1, dot: true });
...
async function getVolumes(volumeStartPath, context) {
  ...
  // return cleanNestedFoldersResults();
