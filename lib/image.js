...
const SF = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'tiff', 'tif', 'mng', 'bmp', 'avif'];
const SUPPORTED_FORMATS = [...SF, ...SF.map((e) => e.toUpperCase())];

async function identifyWithWarnings(imagePath) {
  return new Promise((resolve, reject) => {
    gm(imagePath)
      .command('identify')
      .in('-regard-warnings') // Add the -regard-warnings flag
      .write('null:', (err, stdout) => {
        if (!err) {
          resolve(stdout);
        } else {
          log.debug(`Image Validation for ${imagePath} resulted in '${err}'`);
          reject(err);
        }
      });
  });
...
async function validateImage(imagePath) {
  ...
  */
  ...
  // let response = await LIMIT(() => resizeTest(imagePath, tmp));
  // response = await LIMIT(() => identifyWithWarnings(imagePath, tmp));
  return LIMIT(() => identifyWithWarnings(imagePath));
  // .then(() => identifyWithWarnings(imagePath, tmp));
}
...
async function validateImageDir(dir) {
  ...
  await Promise.all(
    imagesPath.map(async (imagePath) => validateImage(imagePath)
...
async function convertImageFormat(filePath, desiredFormat) {
  const { dir, name } = path.parse(filePath);
  const outputFilePath = path.join(dir, `${name}.${desiredFormat}`);
  const stats = fs.statSync(filePath)
  const fileSizeInBytes = stats.size;

  return new Promise((resolve, reject) => {
    gm(filePath)
      .define(`webp:target-size=${fileSizeInBytes}`)
      // .define(`webp:`)
      // .quality(80)
      // .compress('Lossless')
      ...
      .write(outputFilePath, (err) => {
        if (!err) {
          ...
          console.error(`Error converting image to ${desiredFormat}: ${err}`);
          reject(err);
        }
      });
  });
...
async function convertToWebp(extractedArchiveDir) {
  log.info(`Currently only for Avif: Converting images to Webp in "${extractedArchiveDir}"`);
  const images = await getImages(extractedArchiveDir);
  return Promise.all(
    images.map(
      async (image) => LIMIT(async () => {
        if (path.extname(image).toLowerCase() === '.avif') {
          await convertImageFormat(image, 'webp');
          await rm(image, false);
        }
      }),
    ),
  );
