...
async function needsCover(context) {
  ...
  if (imageExists) {
    log.debug(`Cover image already exists for "${archivePath}`);
    return {};
  }
  return { ...context };
}
...
async function getContextsWithoutCovers(contexts) {
  const contextsWithoutCovers = await Promise.all(
    contexts.map(async (context) => needsCover(context)),
  );
  return contextsWithoutCovers.filter((context) => context.archivePath);
}

async function getCovers(contexts) {
  const validContexts = await getContextsWithoutCovers(contexts);
  const results = await getResults(DOWNLOAD_COVERS, validContexts, getCover);
  return results;
}

module.exports = {
  getContextsWithoutCovers,
  getCovers,
};
