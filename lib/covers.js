...
function getQuery(coverQuery, title, seriesName, volumeNumber, authors = []) {
  ...
  const targeting = coverQuery || `site:${BOOKMETER} ${authors.join(' ')}`;
...
async function needsCover(context, { coverQuery, noCoverValidate }) {
  ...
  return { ...context, coverQuery, noCoverValidate };
}
...
async function getCovers(contexts, config) {
  const contextsWithoutCovers = await Promise.all(
    contexts.map(async (context) => needsCover(context, config)),
  );
