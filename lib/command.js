...
const { isWin, getPathData } = require('./utils');
...
async function zipArchive(dirToArchive, archivePath) {
  ...
  const contentExists = (await glob([{ escape: path.resolve(dirToArchive) }, { raw: '/**/*' }])).length > 0;
  if (!contentExists) {
...
async function getArchives(startPath, extensions = ['7z', 'rar', 'zip', 'cbr', 'cbz'], context = {}) {
  const escapedStartPath = fg.escapePath(path.resolve(startPath));
  ...
  const archives = await fg(globPath, { dot: true, caseSensitiveMatch: false });
