...
function getNewFilePath(archivePath, newExt, junkToFilter) {
  const { fileName, dir } = getPathData(archivePath);
  const newFilePath = `${dir}/${fileName}.${newExt}`;
  const newCleanFilePath = removeJunk(newFilePath, junkToFilter);
  log.debug(`New file would be: "${newCleanFilePath}"`);
  return newCleanFilePath;
}
...
async function getNestedArchivePaths(extractedArchiveDir, rootPath, filterFiles) {
...
async function getNestedArchives(extractedArchiveDir, rootPath, filterFiles) {
  const nestedArchives = await getNestedArchivePaths(extractedArchiveDir, rootPath, filterFiles);
  let wasNested = true;
...
function getRootArchivePath(existingRootPath, archivePath, junkToFilter) {
  if (existingRootPath) {
    const { fileName } = getPathData(archivePath);
    const { dir } = getPathData(existingRootPath);
    return removeJunk(`${dir}/${fileName}.cbz`, junkToFilter);
  }
  return removeJunk(archivePath, junkToFilter);
}
...
async function convertToCBZ(context) {
  const {
    archivePath, rootPath, filesToDelete, junkToFilter, filesToAllow,
  } = context;
  const newFilePath = getNewFilePath(archivePath, 'cbz', junkToFilter);
  ...
  const originalExtractedArchiveDir = await extractArchiveInPlace(archivePath, validArchiveType);
  const {
    extractedArchiveDir,
    unexpectedFiles,
  ...
  const {
    ...
    extractedArchiveDir,
    removeJunk(rootPath || archivePath, junkToFilter),
    { filesToDelete, junkToFilter, filesToAllow },
  );

  let newZipPath = null;
  if (wasNested) {
    newZipPath = extractedArchiveDir;
    updatedContext.rootPath = getRootArchivePath(rootPath, archivePath, junkToFilter);
    ...
    newZipPath = getRootArchivePath(rootPath, newFilePath, junkToFilter);
    await zipAndMove(
      extractedArchiveDir,
      archivePath,
      removeJunk(archivePath, junkToFilter),
      newZipPath,
    );
  }
