...
async function handleNestedArchives(archivePaths) {
  if (!archivePaths.length) {
    log.debug(`No nested archives found in: "${archivePaths}"`);
    return false;
  }
...
async function getNestedArchivePaths(extractedArchiveDir) {
...
async function getNestedArchives(extractedArchiveDir) {
  const nestedArchives = await getNestedArchivePaths(extractedArchiveDir);
...
async function convertToCBZ(context) {
  ...
  const {
    ...
    directChildren,
    wasNested,
  } = await getNestedArchives(extractedArchiveDir);
  let newZipPath = null;
  if (wasNested) {
