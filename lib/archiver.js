...
async function convertToCBZ(context) {
  ...
  const { unexpectedFiles } = cleanedContents;
  let { extractedArchiveDir } = cleanedContents;
  ...
  if (wasNested) {
    ...
    extractedArchiveDir = await enhanceArchiveContents(extractedArchiveDir, context);
    const {
      nestedVolumesResult,
