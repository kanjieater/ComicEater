...
async function addDirToArchive(localDir, archivePath) {
  ...
  if (!isWin()) {
    try {
      sevenZip = await spawn('7z', ['a', archivePath, `${localDir}/*`]);
      log(sevenZip.toString(), 'debug');
    } catch (e) {
      throw new Error(e.stderr.toString());
    }
...
async function extractFileFromArchive(internalFilePath, archivePath, extractPath) {
  ...
  if (!isWin()) {
    ...
    const command = '7z';
    const params = ['e', wrapInQuotes(archivePath), '-aoa', wrapInQuotes(`-o${extractPath}`), wrapInQuotes(internalFilePath)];
    log(`Running command: "${command} ${params.join(' ')}"`);
    await exec(`${command} ${params.join(' ')}`);
    // sevenZip = await spawn(command, params);
    // log('here')
    // log(sevenZip.toString(), 'debug');
    // } catch (e) {
    // throw new Error(e.stderr.toString());
  ...
  const { fileName, ext } = getPathData(archivePath);
...
async function getArchives(startPath) {
  ...
  let globPath = `${escapedStartPath}/**/*.{7z,rar,zip,cbr,cbz}`;
  try {
