...
async function convertToCBZ(context) {
  ...
  const originalArchive = await convertCBZToZip(archivePath, getNewFilePath(archivePath, 'zip', junkToFilter));
  ...
  const {
    ...
    extractedArchiveDir,
    removeJunk(rootPath || originalArchive, junkToFilter),
    context,
  );
  ...
  if (wasNested) {
    ...
    updatedContext.rootPath = getRootArchivePath(rootPath, originalArchive, junkToFilter);
    ...
    await zipAndMove(
      extractedArchiveDir,
      originalArchive,
      removeJunk(originalArchive, junkToFilter),
      newZipPath,
    );
  }
  ...
  return {
    ...
    originalArchivePath: originalArchive,
    unexpectedFiles,
    recordChange: true,
  };
}
