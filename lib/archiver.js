...
const {
  rm, moveFile, removeAllEmptyDirs, checkPathExists,
} = require('./files');
...
async function startMergeSubFoldersSaga(resultsSaga) {
  ...
  async function internalCallback(nestedContext) {
    ...
    async function moveNestedToParent(context) {
      ...
      if (checkPathExists(childFilePath)) {
    ...
    const flatResults = getFlatDirectChildrenResults(nestedContext.nestedArchives.successful);
    // const directChildren = nestedContext.nestedArchives.successful;
    const sagaResults = await getResults(MERGE_NESTED_SUB_FOLDERS, flatResults, moveNestedToParent);
