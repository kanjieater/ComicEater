...
const {
  getNestedVolumes, cleanUpNestedVolume, zipAndMove, validateContents,
} = require('./volumeFinder');
const {
  callProgram, validate7Zip,
  validateRar, validateArchive, getNewFilePath, getArchives, extract7z,
} = require('./command');

const CONVERT_TO_CBZ = 'Convert to CBZ';
const CONVERT_NESTED_TO_CBZ = 'Convert Nested Archives to CBZ';
// const VERIFY_CBZ_CONTENTS = 'Verify CBZ Image Contents';
...
async function extractFileFromArchive(internalFilePath, archivePath, extractPath) {
  ...
  const { stdout } = await callProgram('7z', [
    'e',
  ...
  if (stdout.includes('No files to process')) {
    throw new Error(`7z failed to find the files to extract for "${archivePath}". This could be a bug in 7z.`);
  }
...
async function getFilesWithExt(startPath, extensions) {
  ...
  const allExts = [...extensions, ...extensions.map((e) => e.toUpperCase())];
  return glob([{
    escape: startPath,
  }, {
    raw: '**/*.',
...
async function extractArchiveInPlace(archivePath, validArchiveType, cleanFileName) {
  ...
  const extractPath = path.join(dir, fileName, '/');
...
async function enhanceArchiveContents(extractedArchiveDir, context) {
  ...
  if (context.convertToWebp) {
    await convertToWebp(extractedArchiveDir);
    // enhancements.convertToWebp = true;
  }
  return enhancements;
}
