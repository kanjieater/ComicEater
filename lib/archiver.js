...
const {
  zipAndMove, callProgram, wrapInQuotes, validate7Zip, validateRar, validateArchive, getNewFilePath,
} = require('./command');
...
async function getArchives(startPath, extensions = ['7z', 'rar', 'zip', 'cbr', 'cbz'], context = {}) {
  const escapedStartPath = fg.escapePath(startPath);
  ...
  const archives = await fg(globPath, { dot: true });
...
async function cleanArchiveContents(originalDir, { filesToDelete, junkToFilter, filesToAllow }) {
  ...
  const allFiles = await glob([{ escape: resolve(originalDir) }, { raw: '/**/*' }]);
