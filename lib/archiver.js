...
async function cleanArchiveContents(originalDir, { filesToDelete, junkToFilter, filesToAllow }) {
  ...
  const allFiles = await glob([{ escape: path.resolve(originalDir) }, { raw: '/**/*' }]);
  ...
  await Promise.all(
    zippedFromTo.map(async ([from, to]) => moveFile(from, to, true)),
  );
...
async function convertToCBZ(context) {
  ...
  if (wasNested) {
    ...
    if (wasNestedVolume) {
      ...
      deleteIfSuccessful = path.resolve(originalArchive);
    } else {
      await zipAndMove(
        extractedArchiveDir,
        cleanFilePath,
