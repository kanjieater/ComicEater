require('dotenv').config(); // Do this here so yargs can pick it up
...
async function moveToMaintenance(config) {
  const directoryPath = path.dirname(config.configFile);
  const lockFileName = 'comiceater.lock';
  const lockFilePath = path.join(directoryPath, lockFileName);
  // const lockFilePath = path.join('/lockfile/', 'comiceater.lock'); // Specify the lock file path
...
async function getArchivesFromConfig(config, folder = 'maintenanceFolder') {
  ...
  try {
    ...
    if (err?.code === 'EIO') {
      log.error('Encountered the ambiguous EIO error. One reason, may be due to a directory path being over the OS limit (255 bytes in Ubuntu). Try renaming any extremely long path titles.');
    }
    log.error(err.stack);
  }
  if (!(archives && archives.length)) {
  ...
  const cliOptions = {
    ...
    coverPath:
      typeof argv.downloadCover === 'string'
        ...
        : argv.downloadCover,
    coverQuery: argv.coverQuery,
    noCoverValidate: argv.noCoverValidate,
    upscale: argv.upscale,
    trimWhiteSpace: argv.trimWhiteSpace,
    splitPages: argv.splitPages,
  };
  const config = await getConfig({ configFile, cliOptions }, argv);
