...
async function getRemainingFolders(queueFolders) {
  const arrays = await Promise.all(
    queueFolders.map(
      async (queueFolder) => ({
        ...queueFolder,
        filePaths: [
          ...(await getAllDirs(queueFolder.queuePath)),
        ],
      }),
    ),
  );
...
async function moveToMaintenance(config) {
  ...
  const remainingFolders = await getRemainingFolders(config.queueFolders);
  // Split these up so that a file being moved doesn't conflict with a dir in parallel
  const results = await moveFilesToMaintenanceSaga(remainingFiles);
  const resultsDirs = await moveFilesToMaintenanceSaga(remainingFolders);
  return [results, resultsDirs];
}
