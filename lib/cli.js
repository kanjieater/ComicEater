...
async function getCoversCLI(config, contexts = []) {
  let archives = contexts.filter((context) => context?.shouldDownloadCover);
  if (archives.length === 0) {
    const rawArchives = await getArchivesCLI(config.coverPath, undefined, config.seriesFolders[0]);
    archives = (await getSeries(rawArchives)).successful;
  }

  const coverResults = await getCovers(archives);
...
async function convertToSeriesCLI(archives, config) {
  const { offline, downloadCover } = config;
  const identifiedSeriesMetaResults = await getSeries(archives);
...
async function parseCommands() {
  ...
  const cliOptions = {
    ...
    shouldDownloadCover: !!argv.downloadCover || false,
    coverPath: getCleanPath(argv.downloadCover),
  };
