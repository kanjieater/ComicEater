...
async function maintainCollectionCLI(config) {
  ...
  const taggedCoverResults = await taggedCoversCLI(config, seriesResults.latestResults.successful);
  const komgaResults = await addToKomgaCLI(config, seriesResults.latestResults.successful);
