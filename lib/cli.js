...
async function getArchivesArrays(folders, altPath) {
  const arrays = await Promise.all(
    folders.map(async (folder) => {
      const archives = await getArchives(folder[altPath], undefined, folder);
      const volumeStartPaths = await getVolumeStartPaths(folder[altPath], folder);
...
async function convertToSeriesCLI(config) {
  ...
  allResultsLogs.push(getResultLog(identifiedSeriesMetaResults));
...
async function parseCommands() {
  ...
  if (argv.convertToCBZ) {
    ...
    resultsAndLogs = await convertToSeriesCLI(config);
    shouldCleanUp = true;
