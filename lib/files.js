...
const FILE_LIMIT = getCPULimit();
const GLOB_LIMIT = pLimit(FILE_LIMIT * 4);
const MOVE_LIMIT = pLimit(FILE_LIMIT * 4);
const RM_LIMIT = pLimit(FILE_LIMIT / 4);
...
async function glob(input, options = {}) {
  ...
  return GLOB_LIMIT(() => fg(escapedInput, { ...baseOptions, ...options }));
}
...
//     setTimeout(() => reject(new Error(`File removal timed out for ${filePath}`)), TIMEOUT_MS);
//   });

//   try {
//     await RM_LIMIT(() => Promise.race([fs.remove(filePath), timeoutPromise]));
...
async function rm(filePath, dryRun = true) {
  ...
  try {
    // await RM_LIMIT(() => fs.remove(filePath));
...
async function removeEmptyDirs(folder, maxPath = '') {
  ...
  if (files.length > 0) {
    ...
    || (files.length === 1 && files[0] === 'desktop.ini')) {
...
async function moveFileUnlessExists(from, to, overwrite, shouldLog) {
  try {
    return await moveFile(from, to, overwrite, shouldLog);
  } catch (e) {
    if (e?.code === 'EIO') {
      log.debug(`Skipping ${from}" to "${to}" because it exists already.`);
      await fs.promises.unlink(from);
    } else {
      throw new Error(e);
    }
...
async function moveFilesBatch(zippedFromTo) {
  return Promise.all(
    zippedFromTo.map(async ([from, to]) => MOVE_LIMIT(
      async () => moveFileUnlessExists(from, to, false, false),
    )),
  );
}
...
async function removeDirsSaga(context) {
  ...
  try {
    ...
    // Since we're calling this multiple times it might have already been removed
    // TODO: cleanup all timestamped queue folders non-recursively if non-empty
    if (e.code !== 'ENOENT' && e.code !== 'EIO') {
      throw new Error(e);
    }
  }
  return {
...
function shouldMoveBackToQueue(unsuccessful) {
  if (unsuccessful.length !== 0) {
    return unsuccessful.some((context) => {
      if (context?.error?.message?.includes('EACCES')) {
        return true;
      }
    });
  }
...
async function moveFilesToMaintenanceSaga(contexts) {
  ...
  if (shouldMoveBackToQueue(results.unsuccessful)) {
    log.error('Failed to move to Maintenance folder. Moving all files back to Queue.');
    const fromToMap = fileContexts.map(
      ({ maintenancePath, filePath }) => [maintenancePath, filePath],
    );
    await moveFilesBatch(fromToMap);
  }
