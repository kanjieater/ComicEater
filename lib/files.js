...
async function moveFile(from, to, overwrite = false) {
  ...
  try {
    ...
    //   throw new Error(`EACCES: permission denied, mkdir '${dir}'`);
    // }
    throw new Error(e);
  }
}
...
async function cleanPath(context) {
  await rm(context.deleteIfSuccessful, true);
  return {
    ...context,
...
function getTopLevelPathContexts(contexts) {
  const topLevelPaths = {};
  contexts.forEach((context) => {
    if (Object.keys(topLevelPaths).length === 0) {
      topLevelPaths[context.deleteIfSuccessful] = context;
    } else {
      let shouldAdd = true;
      Object.keys(topLevelPaths).forEach((topLevelPath) => {
        if (context.deleteIfSuccessful.includes(topLevelPath)) {
          delete topLevelPaths[topLevelPath];
          topLevelPaths[context.deleteIfSuccessful] = context;
        }
        if (topLevelPath.includes(context.deleteIfSuccessful)) {
          shouldAdd = false;
        }
      });
      if (shouldAdd) {
...
async function cleanSuccessfulResults(existingResults) {
  ...
  allSuccessful.forEach((context) => {
    if (context.deleteIfSuccessful) {
      pathsToDelete[context.deleteIfSuccessful] = context;
    }
  });
  const topLevelPaths = Object.values(getTopLevelPathContexts(Object.values(pathsToDelete)));

  const results = await getResults(CLEAN_UP_SUCCESSFUL, topLevelPaths, cleanPath);
  return results;
}
