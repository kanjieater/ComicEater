...
const MOVE_FILES_TO_MAINTENANCE = 'Check Queues and Move Remaining Files to Maintenance Folder';
...
async function moveFileToMaintenance(context) {
  await moveFile(context.filePath, context.targetPath, false, false);
  return {
    ...context,
...
async function removeDirsSaga(context) {
  ...
  const maintenanceDirsToRemove = await getChildDirs(context.maintenanceFolder);
  const dirsToRemove = [...queueDirsToRemove];
  await Promise.all(dirsToRemove.map(async (dir) => removeEmptyDirs(dir)));

  await Promise.all(maintenanceDirsToRemove.map(async (dir) => removeEmptyDirs(dir)));
...
function getBackupTargetFilePath(filePath, maintenanceFolder, queuePath, date) {
  const relativeFilePath = filePath.replace(queuePath, '');
  const newFilePath = `${path.resolve(maintenanceFolder)}/${date}/${relativeFilePath}`;
  return newFilePath;
}
    ...
    context.filePaths.forEach((filePath) => {
      fileContexts.push({
        ...
        targetPath: getBackupTargetFilePath(
          filePath, context.maintenanceFolder, context.queuePath, date,
        ),
      });
    });
  });
