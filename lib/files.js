...
const { getPathData, glob } = require('./utils');
...
async function moveFileToMaintenance(context) {
  await moveFile(context.filePath, context.targetPath, true);
  return {
    ...context,
...
function getTargeFilePath(filePath, maintenanceFolder, queuePath) {
  const relativeFilePath = filePath.replace(queuePath, '');
  const newFilePath = `${path.resolve(maintenanceFolder)}/${relativeFilePath}`;
  return newFilePath;
}
  ...
  contexts.forEach((context) => {
    context.filePaths.forEach((filePath) => {
      fileContexts.push({
        ...context,
        filePath,
        targetPath: getTargeFilePath(filePath, context.maintenanceFolder, context.queuePath),
      });
    });
  });
